...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        TH_H2 = self.config.TH_H2
        TH_L2 = self.config.TH_L2
        TH_H3 = self.config.TH_H3
        TH_L3 = self.config.TH_L3
        TH_H4 = self.config.TH_H4
        TH_L4 = self.config.TH_L4
        ...
        #4bit int
        tmp_attn = torch.clamp(torch.round(tmp_attn * 16), 0, 15)
        tmp_sum = torch.sum(tmp_attn, dim=-2)
        ...
        return attn_output, [token_life,tmp_attn], past_key_value
