...
class TFViTMAEForPreTraining(TFViTMAEPreTrainedModel):
    ...
    def patchify(self, pixel_values, interpolate_pos_encoding=False):
        ...
        num_patches_one_direction = shape_list(pixel_values)[1] // patch_size
        num_patches_two_direction = shape_list(pixel_values)[2] // patch_size
        patchified_pixel_values = tf.reshape(
            pixel_values,
            (batch_size, num_patches_one_direction, patch_size, num_patches_two_direction, patch_size, num_channels),
        )
    ...
    def unpatchify(self, patchified_pixel_values, pixel_values):
        """
        Args:
        ...
        num_patches_two_direction = shape_list(pixel_values)[3] // patch_size
        # sanity check
        tf.debugging.assert_equal(
            num_patches_one_direction * num_patches_two_direction,
        ...
        patchified_pixel_values = tf.reshape(
            patchified_pixel_values,
            (batch_size, num_patches_one_direction, num_patches_two_direction, patch_size, patch_size, num_channels),
        )
